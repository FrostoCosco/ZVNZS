...
excluded_dirs = ["Savegames", "Screenshots"]
...
# Extract the zip file, skipping the excluded directories
with zipfile.ZipFile(output, 'r') as zip_ref:
    for member in zip_ref.namelist():
        if not should_skip(member):
            zip_ref.extract(member, '.')

print('Extraction complete. Excluded directories have not been overwritten.')
